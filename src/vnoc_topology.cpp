...
bool TOPOLOGY::parse_command_arguments( int argc, char *argv[])
{
    ...
    while ( i < argc) {
        ...
        if (strcmp (argv[i],"traffic:") == 0) {
            ...
            //milad add
            } else if (strcmp(argv[i+1], "IPCORE") == 0) {
                _traffic_type = IPCORE_TRAFFIC;

            } else {
                printf("Error:  traffic must be UNIFORM, HOTSPOT, TRANSPOSE1, TRANSPOSE2, SELFSIMILAR, TRACEFILE or IPCORE.\n");
                exit (1);
            }
            i += 2;
            continue;
        ...
        if ( !strcmp(argv[i], "cycles:")) {
            ...
            if (_simulation_cycles_count <= 0 || _simulation_cycles_count > 5000000000) { 
                printf("Error:  cycles value must be between [1 5000000].\n");
                exit(1); 
            }
            i += 2; 
            continue;
