...
void EVENT_QUEUE::insert_initial_events()
{
    ...
    if ( _vnoc->topology()->dvfs_mode() == SYNC) {
        ...
        //printf ("add_event( EVENT(EVENT::SYNC_PREDICT_DVFS_SET\n");
        //cin.get();
...
{
    //milad debug
    cout << "EVENT_QUEUE::run_simulation activated"<<endl;
    ...
    //milad comment:
    //waiting for events produced by IP_COREs
    //if send_request
    //  build an event + take the name of IP_CORE + take the packet
    ...
    while ( _events.size() > 0 ) {//&& _current_sim_time <= simulation_cycles_count
        ...
        //milad debug  
        //cout << "event_queue_size= " << event_queue_size()<<endl;        
        //cout << "_current_sim_time= "<<_current_sim_time<<endl;        
        //cout << "this_event.start_time= "<<this_event.start_time()<<endl;
        //if (_current_sim_time>1)
        //    cin.get();
        ...
        //milad debug
        //cout << "before switch ( this_event.type())"<<"this_event.type()= "<<this_event.type()<<endl;
        switch ( this_event.type()) {
            //milad debug
            //cout << "switch ( this_event.type()) activated"<<endl;
            ...
            case EVENT::PE :
                ...
                //cout << "EVENT::PE"<< endl;
                _vnoc->receive_EVENT_PE( this_event); // go to vnoc.cpp
            ...
            case EVENT::SYNC_PREDICT_DVFS_SET :
                ...
                //cout << "EVENT::SYNC_PREDICT_DVFS_SET"<<endl;
                _vnoc->receive_EVENT_SYNC_PREDICT_DVFS_SET( this_event);
                //cin.get();
                break;

            case EVENT::LINK :
                //cout << "EVENT::LINK"<<endl;
            ...
            case EVENT::CREDIT :
                ...
                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_CREDIT( this_event);
                //cin.get();                
                break;
            /*
            case EVENT::TO_GET_NALU_FIRST_RUN :

                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_TO_GET_NALU_FIRST_RUN(this_event);
                //cin.get();                
                break;
            */
            case EVENT::TO_GET_NALU :

                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_TO_GET_NALU(this_event);
                //cin.get();                
                break;
            
            case EVENT::GET_NALU_TO_DECODE_HDR_FIRST_RUN :

                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_GET_NALU_TO_DECODE_HDR_FIRST_RUN(this_event);
                //cin.get();                
                break;

            case EVENT::GET_NALU_TO_DECODE_HDR :

                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_GET_NALU_TO_DECODE_HDR(this_event);
                //cin.get();                
                break;


            case EVENT::DECODE_HDR_TO_DECODE_MB_FIRST_RUN :

                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_DECODE_HDR_TO_DECODE_MB_FIRST_RUN(this_event);
                //cin.get();                
                break;

            case EVENT::DECODE_MB_TO_INTRA_PRED :

                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_DECODE_MB_TO_INTRA_PRED(this_event);
                //cin.get();                
                break;

            case EVENT::DECODE_MB_TO_INTER_PRED_NORMAL :

                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_DECODE_MB_TO_INTER_PRED_NORMAL(this_event);
                //cin.get();                
                break;

            case EVENT::DECODE_MB_TO_INTER_PRED_PSKIP :

                //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_DECODE_MB_TO_INTER_PRED_PSKIP(this_event);
                //cin.get();                
                break;

            case EVENT::INTRA_PRED_MB_FRAME_TO_INTER_PRED :
               //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_INTRA_PRED_MB_FRAME_TO_INTER_PRED(this_event);
                //cin.get();                
                break;
            
            case EVENT::INTER_PRED_MB_FRAME_TO_INTRA_PRED :
               //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_INTER_PRED_MB_FRAME_TO_INTRA_PRED(this_event);
                //cin.get();                
                break;

            case EVENT::INTRA_PRED_MB_FRAME_TO_FRAME_BUFFER :
               //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_INTRA_PRED_MB_FRAME_TO_FRAME_BUFFER(this_event);
                //cin.get();                
                break;
            
            case EVENT::INTER_PRED_MB_FRAME_TO_FRAME_BUFFER :
               //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_INTER_PRED_MB_FRAME_TO_FRAME_BUFFER(this_event);
                //cin.get();                
                break;

            case EVENT::FRAME_BUFFER_MB_FRAME_TO_DISPLAY :
               //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_FRAME_BUFFER_MB_FRAME_TO_DISPLAY(this_event);
                //cin.get();                
                break;

            case EVENT::TO_CONTINUE_DECODE_MB :
               //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_TO_CONTINUE_DECODE_MB(this_event);
                //cin.get();                
                break;

            case EVENT::UPDATE_REF_FRAME_IN_INTER_PRED :
               //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_UPDATE_REF_FRAME_IN_INTER_PRED(this_event);
                //cin.get();                
                break;

            case EVENT::UPDATE_FRAME_IN_FRAME_BUFFER :
               //cout << "EVENT::CREDIT"<<endl;
                _vnoc->receive_EVENT_UPDATE_FRAME_IN_FRAME_BUFFER(this_event);
                //cin.get();                
                break;
             ...
             default:
                //printf("DUMMY\n");
                assert(0);
                //cout << "EVENT::default  activated"<<endl;
                break;
        ...
        //ROUTER A=_vnoc->router(AD);
        
        //ROUTER_OUTPUT test_r_out= A.output();
        ...
        cout << "_current_sim_time= "<<_current_sim_time<<endl;        
        //cout << "this_event.start_time= "<<this_event.start_time()<<endl;
        cout << "simulation_cycles_count= "<< simulation_cycles_count<<endl;
