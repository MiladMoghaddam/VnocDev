...
NOC_interface_receive noc_ifc_R0(mpack,6);
NOC_interface_receive noc_ifc_R1(mpack,6);
NOC_interface_receive noc_ifc_R2(mpack,6);
NOC_interface_receive noc_ifc_R3(mpack,6);
*/
...
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
long ROUTER::receive_packet_from_local_PE(DATA packet_data,ADDRESS &des_addr) // called by vnoc
{
    ...
    _local_injection_time=injection_time; 



        
    int inj_time = _local_injection_time;        
    //fprintf( InFile ,"#%d, Time:%d, Data(%d,%d,%d,%d,%d,%d)\n",_inj_packet_counter,inj_time,packet_data[0],packet_data[1],packet_data[2],packet_data[3],packet_data[4],packet_data[5]);
        
        
    inject_packet_by_PE( _inj_packet_counter,
                _address, 
                des_addr, 
                _local_injection_time, packet_size_g, packet_data);
            //cout << "ROUTER::receive_packet_from_local_IP_CORE::AFTER inject_packet_by_IP_CORE" <<endl;
...
void ROUTER::inject_packet_by_PE( long flit_id, ADDRESS &sor_addr, ADDRESS &des_addr,
                                       double time, long packet_size, DATA packet_data)
{
    VC_PAIR vc_pair;
    //cout milad
    //cout <<endl<<"----------Packet_size= "<<packet_size<<"-----Time= "<<time<<endl;

    for ( long l = 0; l < packet_size; l++) {
        
        DATA flit_data; // vector<unsigned long long>;
        for ( long i = 0; i < _flit_size; i++) {
            //printf ("packet_data[%d]= %d\n",l,packet_data[l]);            
            _init_data[i] = static_cast<DATA_ATOMIC_UNIT>(packet_data[l]);        
        ...
        if ( l == 0) {
            ...
            //milad
            FLIT my_FLIT;
            my_FLIT=FLIT(flit_id, FLIT::HEADER, sor_addr, des_addr, time, flit_data);
            //cout <<"HEADER"<<endl;
            //my_FLIT.Display();            
        }
            ...
            //milad
            FLIT my_FLIT;
            my_FLIT=FLIT(flit_id, FLIT::BODY, sor_addr, des_addr, time, flit_data);
            //cout <<"BODY"<<endl;
            //my_FLIT.Display();
            ...
            //milad
            FLIT my_FLIT;
            my_FLIT=FLIT(flit_id, FLIT::TAIL, sor_addr, des_addr, time, flit_data);
            //cout <<"TAIL"<<endl;
            //my_FLIT.Display();
...
bool ROUTER::receive_packet_from_local_traffic_injector(long dest_id)
{
    ...
    if ( _input.injection_buff_full() == false) {
        ...
        //printf("_input.injection_buff_full() == true");
...
//milad add
//bool ROUTER::receive_packet_from_local_traffic_injector_by_IP_CORE(long dest_id )
//{
//    // retrieve packets injected by the traffic_injector hooked
//    // to this router and generated with a certain traffic pattern;
//    // this function is called by:
...
//}
///////////////////////////
long ROUTER::receive_packet_from_local_IP_CORE(DATA packet_data) // called by vnoc
{
    // retrieve packets from individual trace files named for example
    // like: tests/bench.x.y
    // these files have the injection information only for router at address x,y;
    ...
    // code difficult to understand;
        ...
        _local_injection_time=injection_time; 
        //src_addr.push_back(1);
        //src_addr.push_back(1);
        des_addr.push_back(2);
        des_addr.push_back(2);
    
        packet_size=6;
        /*
        packet_data.push_back(1111);
        packet_data.push_back(2222);
        packet_data.push_back(3333);
        packet_data.push_back(4444);
        packet_data.push_back(5555);
        packet_data.push_back(6666);
        */
        ...
        fprintf( InFile ,"#%d, Time:%d, Data(%d,%d,%d,%d,%d,%d)\n",_inj_packet_counter,inj_time,packet_data[0],packet_data[1],packet_data[2],packet_data[3],packet_data[4],packet_data[5]);
            ...
            inject_packet_by_IP_CORE( _inj_packet_counter,
                _address, 
                des_addr, 
                _local_injection_time, packet_size, packet_data);
            //cout << "ROUTER::receive_packet_from_local_IP_CORE::AFTER inject_packet_by_IP_CORE" <<endl;
...
void ROUTER::inject_packet_by_IP_CORE( long flit_id, ADDRESS &sor_addr, ADDRESS &des_addr,
                                       double time, long packet_size, DATA packet_data)
{
    VC_PAIR vc_pair;
    //cout milad
    ...
    for ( long l = 0; l < packet_size; l++) {
        ...
        for ( long i = 0; i < _flit_size; i++) {
            ...
            //milad
            //cout <<"_init_data["<<i<<"],l["<<l<<"]= "<< _init_data[i] <<"\n";
            //cout <<"CORR_EFF= "<<CORR_EFF<<endl;
            //cout <<"_vnoc->topology()->rng().flat_ull(0, ULLONG_MAX)= "<< _vnoc->topology()->rng().flat_ull(0, ULLONG_MAX) << endl;
            //milad change
            _init_data[i] = static_cast<DATA_ATOMIC_UNIT>(packet_data[l]);
        
            //milad cout
            //cout <<"Final _init_data["<<i<<"],l["<<l<<"]= "<< _init_data[i] <<"\n";            
            flit_data.push_back( _init_data[i]);
            //milad
            //DATA::const_iterator first = flit_data.begin();
            //DATA::const_iterator last = flit_data.begin();
            //cout <<flit_data.size();
                            ...
                            double time, long packet_size)
{
    VC_PAIR vc_pair;
    //cout milad
    cout <<endl<<"----------Packet_size= "<<packet_size<<"----------"<<endl;
    ...
    for ( long l = 0; l < packet_size; l++) {
        ...
        if ( l == 0) {
            ...
            // if the input buffer is empty, set it to be ROUTING;
            if ( _input.input_buff(0, vc_pair.first).size() == 0) {
                //cout <<"3 not UNIFORM: vc_state: ROUTING"<<endl;
...
void ROUTER::receive_flit_from_upstream(long port_id, long vc_id, FLIT &flit)
{
    ...
    if ( flit.type() == FLIT::HEADER) {
        if ( _input.input_buff(port_id, vc_id).size() == 1) {
            ...
            //milad debug
            //cout << "ROUTER::receive_flit_from_upstream go to ROUTING"<<endl;            
            //cin.get();
        ...
        if ( _input.vc_state(port_id, vc_id) == IDLE) {
            //milad debug
            //cout << "ROUTER::receive_flit_from_upstream _input.vc_state_update( port_id, vc_id, SW_AB); from IDLE"<<endl;            
            //cin.get();
...
{
    //cout <<endl<<"SIMULATE ONE ROUTER"<<endl;
    
    // simulate all router's pipeline stages;

    //cout << "ROUTER::simulate_one_router() inside"<<endl;
...
void ROUTER::send_flit_via_physical_link( long i)
{
    ...
    //cout<<"Debug::ROUTER::send_flit_via_physical_link"<<endl;
    //cin.get();    
    if ( _output.out_buffer(i).size() > 0) {
        //cout<<"_output.out_buffer(i).size()= "<<_output.out_buffer(i).size() <<endl;
        //cin.get();
        ...
        //cout << "7:  _output.get_flit("<<i<<"):"<<endl;  
        //cout << "7 router = "<<id()<<endl;
        //        if (id()==2) 
        //    {
    ...
    for ( long i = 0; i < _physical_ports_count; i++) {
        for ( long j = 0; j < _vc_number; j++) {
            if ( _input.vc_state(i, j) == SW_TR) {
                //milad debug
                //cout << "if ( _input.vc_state(i, j) == SW_TR) { activated"<<endl;
                ...
                //milad debug
                //FLIT test_flit(_output.get_flit(sel_routing.first));
                //cout <<"6: DEBUG: ROUTER::send_flit_to_out_buffer_SW_TR()"<<endl;                
                //cout <<"6: add_flit::sel_routing.first: "<<sel_routing.first<<endl;
                ...
                /*
                if ( i == 0) {
                    if ( _vnoc->traffic_type() == IPCORE_TRAFFIC) {
                        if ( _input.injection_buff_full() == true) {
                            if ( _input.input_buff(0,j).size() < BUFF_BOUND) {
                                _input.clear_injection_buff_full();
                                receive_packet_from_local_IP_CORE(failed_packet);
                            }
                        }
                    }
                }
...
void ROUTER::sw_arbitration_stage_SW_AB() 
{
    ...
    //cout << "1: Debug: _physical_ports_count= "<<_physical_ports_count<<"_vc_number= "<<_vc_number<<endl;
    //cin.get();
    
    for ( long i = 0; i < _physical_ports_count; i++) {
        vector<long> vc_i_t;
        for ( long j = 0; j < _vc_number; j++) {
            //cout << "2: _input.vc_state(" <<i<<","<<j<<")= "<<_input.vc_state(i, j)<<endl;
            //cin.get();
            
            if ( _input.vc_state(i, j) == SW_AB) {
                //milad debig
                //cout <<"if ( _input.vc_state(i, j) == SW_AB)" <<endl;
                //cin.get();
                ...
                if (( _output.counter_next_r(out_t.first, out_t.second) > 0) &&
                    ...
                    //cout <<"if ( _input.vc_state(i, j) == SW_AB) ::vc_i_t.push_back(j) " <<endl;
                    //cin.get();
    ...
    //cout <<"vc_o_map.size() = "<<vc_o_map.size() <<endl;
    //cin.get();
    
    if ( vc_o_map.size() == 0) {
        //cout <<"if ( vc_o_map.size() == 0) RETURN"<<endl;
        //cin.get();
    ...
    //cout <<"_physical_ports_count= "<<_physical_ports_count<<endl;
    //cin.get();
    
    for ( long i = 0; i < _physical_ports_count; i++) {
        long vc_size_t = vc_o_map[i].size();

        //cout <<"before vc_size_t > 0"<<endl;
        //cin.get();
        ...
        if ( vc_size_t > 0) {
            ...
            //milad debug
            //cout <<"_input.vc_state_update(vc_win.first, vc_win.second, SW_TR);"<<endl;
            //cin.get(); 
...
void ROUTER::routing_decision_stage_RC()
{
    ...
    long ary_size = _vnoc->_topology->ary_size();
    double event_time = _vnoc->event_queue()->current_sim_time();
    ...
    for ( long i = 1; i < _physical_ports_count; i++) {
        for ( long j = 0; j < _vc_number; j++) {
            ...
            if ( _input.input_buff(i,j).size() > 0) {
                //milad debug
                //cout<<"debug: ROUTER::routing_decision_stage_RC() _input.input_buff(i,j).size() > 0"<<endl;
                //cin.get();
                ...
                ADDRESS flit_dest = flit_t.des_addr();

                ADDRESS address_GET_NALU;
                address_GET_NALU.push_back(GET_NALU_ID / ary_size);
                address_GET_NALU.push_back(GET_NALU_ID % ary_size);
                ADDRESS address_DECODE_HDR;
                address_DECODE_HDR.push_back(DECODE_HDR_ID / ary_size);
                address_DECODE_HDR.push_back(DECODE_HDR_ID % ary_size);
                ADDRESS address_DECODE_MB;
                address_DECODE_MB.push_back(DECODE_MB_ID / ary_size);
                address_DECODE_MB.push_back(DECODE_MB_ID % ary_size);
                ADDRESS address_INTRA_PRED;
                address_INTRA_PRED.push_back(INTRA_PRED_ID / ary_size);
                address_INTRA_PRED.push_back(INTRA_PRED_ID % ary_size);
                ADDRESS address_INTER_PRED;
                address_INTER_PRED.push_back(INTER_PRED_ID / ary_size);
                address_INTER_PRED.push_back(INTER_PRED_ID % ary_size);
                ADDRESS address_FRAME_BUFFER;
                address_FRAME_BUFFER.push_back(FRAME_BUFFER_ID / ary_size);
                address_FRAME_BUFFER.push_back(FRAME_BUFFER_ID % ary_size);
                ADDRESS address_DISPLAY;
                address_DISPLAY.push_back(DISPLAY_ID / ary_size);
                address_DISPLAY.push_back(DISPLAY_ID % ary_size);
                ADDRESS address_R7;
                address_R7.push_back(1);
                address_R7.push_back(2);
                ADDRESS address_R8;
                address_R8.push_back(2);
                address_R8.push_back(2);

                if (id()==GET_NALU_ID && flit_dest==address_GET_NALU )
                {
                            
                    //cout << "get_packet from router id()=0 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                    
                    _vnoc->_noc_ifc_receive[GET_NALU_ID].get_packet_from_NOC(flit_t.start_time(),flit_data[0]);                    
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[0].noc_ifc_R0.packet_array_display();                    
                    //cin.get();
                }
                
                if (id()==DECODE_HDR_ID && flit_dest==address_DECODE_HDR)
                {

                    //cout << "get_packet from router id()=1 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                    
                    _vnoc->_noc_ifc_receive[DECODE_HDR_ID].get_packet_from_NOC(flit_t.start_time(),flit_data[0]);                    
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[1].packet_array_display();                    
                    //cin.get();

                }
                
                if (id()==DECODE_MB_ID && flit_dest==address_DECODE_MB)
                {
                            
                    //cout << "get_packet from router id()=2 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                    
                    _vnoc->_noc_ifc_receive[DECODE_MB_ID].get_packet_from_NOC(flit_t.start_time(),flit_data[0]);                    
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[2].packet_array_display();                    
                    //cin.get();
                }
                

                if (id()==INTRA_PRED_ID && flit_dest==address_INTRA_PRED)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[INTRA_PRED_ID].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                if (id()==INTER_PRED_ID && flit_dest==address_INTER_PRED)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[INTER_PRED_ID].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                if (id()==FRAME_BUFFER_ID && flit_dest==address_FRAME_BUFFER)
                {                                    
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[FRAME_BUFFER_ID].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                if (id()==DISPLAY_ID && flit_dest==address_DISPLAY)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[DISPLAY_ID].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                /*
                if (id()==7 && flit_dest==address_R7)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[7].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                if (id()==8 && flit_dest==address_R8)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[8].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 


                    int flit_stime= flit_t.start_time();
                    
                    fprintf(OutFile,"flit: %d, sTime: %d, fTime: %.2f\n",flit_data[0],flit_stime,event_time);

                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[8].packet_array_display();                    
                ...
                */
                ...
                /*
                ADDRESS address_R0;
                address_R0.push_back(0);
                address_R0.push_back(0);
                ADDRESS address_R1;
                address_R1.push_back(1);
                address_R1.push_back(0);
                ADDRESS address_R2;
                address_R2.push_back(2);
                address_R2.push_back(0);
                ADDRESS address_R3;
                address_R3.push_back(0);
                address_R3.push_back(1);
                ADDRESS address_R4;
                address_R4.push_back(1);
                address_R4.push_back(1);
                ADDRESS address_R5;
                address_R5.push_back(2);
                address_R5.push_back(1);
                ADDRESS address_R6;
                address_R6.push_back(0);
                address_R6.push_back(2);
                ADDRESS address_R7;
                address_R7.push_back(1);
                address_R7.push_back(2);
                ADDRESS address_R8;
                ...
                address_R8.push_back(2);
                //ADDRESS address_R63;
                //address_R63.push_back(7);
                //address_R63.push_back(7);
                
                                
                if (id()==0 && flit_dest==address_R0 )
                {
                            
                    cout << "get_packet from router id()=0 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                    
                    _vnoc->_noc_ifc_receive[0].get_packet_from_NOC(flit_t.start_time(),flit_data[0]);                    
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[0].noc_ifc_R0.packet_array_display();                    
                    //cin.get();
                }
                
                if (id()==1 && flit_dest==address_R1)
                {

                    cout << "get_packet from router id()=1 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                    
                    _vnoc->_noc_ifc_receive[1].get_packet_from_NOC(flit_t.start_time(),flit_data[0]);                    
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[1].packet_array_display();                    
                    //cin.get();
                ...
                if (id()==2 && flit_dest==address_R2)
                {
                            
                    cout << "get_packet from router id()=2 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                    
                    _vnoc->_noc_ifc_receive[2].get_packet_from_NOC(flit_t.start_time(),flit_data[0]);                    
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[2].packet_array_display();                    
                    //cin.get();
                }
                ...
                if (id()==3 && flit_dest==address_R3)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[3].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                if (id()==4 && flit_dest==address_R4)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[4].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                if (id()==5 && flit_dest==address_R5)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[5].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                if (id()==6 && flit_dest==address_R6)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[6].get_packet_from_NOC(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[3].packet_array_display();                    
                    //cin.get();
                }
                ...
                */
                ...
                /* 
               if (id()==63 && flit_dest==address_R63)
                {
                    //cout << "get_packet from router id()=3 "<<endl;                
                    
                    DATA flit_data=flit_t.data();                                        
                    _vnoc->_noc_ifc_receive[63].get_packet_from_DECODER(flit_t.start_time(),flit_data[0]); 
                    //flit_t.Display();
                    //_vnoc->_noc_ifc_receive[8].packet_array_display();                    
            ...
            if ( _input.vc_state(i, j) == ROUTING) {
               flit_t = _input.get_flit(i, j);
               //if (id()==3){  
                   //cout << "12: header: routing decision: id: "<<id()<<endl;               
                   //flit_t.Display();
                   //cin.get();
                   //}
            ...
            // BODY or TAIL flits;
                ...
                if ( _input.input_buff(i, j).size() > 0) {
                    flit_t = _input.get_flit(i, j);
                    //if (id()==3){                    
                      //cout << "13: body tail: routing decision: id: "<<id()<<endl;               
                      //flit_t.Display(); 
                      //cin.get();
                      //}
