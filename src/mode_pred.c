...
int dump_mpi(mode_pred_info mpi){

	printf("\n\n");	
	printf("dump_mpi\n");
	printf("per-macroblock information     (16x16)\n");
	printf("MbWidth= %d MbHeight=%d MbPitch=%d\n",mpi.MbWidth,mpi.MbHeight,mpi.MbPitch);
	printf("MbMode=\n");
	for (int i=0; i<mpi.MbHeight; i++)
		for (int j=0; j<mpi.MbWidth; j++)	
			printf(" %d,",mpi.MbMode[j+i*mpi.MbWidth] );
	printf("\n");	

	printf("\nper-chroma block information    (8x8)\n");
	printf("CbWidth= %d CbHeight=%d CbPitch=%d\n",mpi.CbWidth,mpi.CbHeight,mpi.CbPitch);
	printf("TotalCoeffC[0]=\n");
	for (int i=0; i<mpi.CbHeight; i++)
		for (int j=0; j<mpi.CbWidth; j++)	
			printf(" %d,",mpi.TotalCoeffC[0][j+i*mpi.CbWidth] );
	printf("\nTotalCoeffC[1]= \n");
	for (int i=0; i<mpi.CbHeight; i++)
		for (int j=0; j<mpi.CbWidth; j++)	
			printf(" %d,",mpi.TotalCoeffC[1][j+i*mpi.CbWidth] );


	printf("\nper-transform block information (4x4)\n");
	printf("TbWidth= %d TbHeight=%d TbPitch=%d\n",mpi.TbWidth,mpi.TbHeight,mpi.TbPitch);
	printf("TotalCoeffL=\n");
	for (int i=0; i<mpi.TbHeight; i++)
		for (int j=0; j<mpi.TbWidth; j++)	
			printf(" %d,",mpi.TotalCoeffL[j+i*mpi.TbWidth] );
	
	printf("\nIntra4x4PredMode= \n");
	for (int i=0; i<mpi.TbHeight; i++)
		for (int j=0; j<mpi.TbWidth; j++)	
			printf(" %d,",mpi.Intra4x4PredMode[j+i*mpi.TbWidth] );
	
	printf("\nMVx=\n");
	for (int i=0; i<mpi.TbHeight; i++)
		for (int j=0; j<mpi.TbWidth; j++)	
			printf(" %d,",mpi.MVx[j+i*mpi.TbWidth]);
	
	printf("\nMVy=\n");	
	for (int i=0; i<mpi.TbHeight; i++)
		for (int j=0; j<mpi.TbWidth; j++)	
			printf(" %d,",mpi.MVy[j+i*mpi.TbWidth]);
	
	printf("\n\n");
}

/*
mode_pred_info my_alloc_mode_pred_info(int width, int height) {
  mode_pred_info *mpi;
  *mpi=calloc(1,sizeof(mode_pred_info));
  int x,y;
...
void clear_mode_pred_info(mode_pred_info *mpi) {
  ...
  if(mpi->MVx)              memset(mpi->MVx,MV_NA&0xFF,mpi->TbPitch*mpi->TbHeight*sizeof(int));//MV_NA&0xFF
  if(mpi->MVy)              memset(mpi->MVy,MV_NA&0xFF,mpi->TbPitch*mpi->TbHeight*sizeof(int));//MV_NA&0xFF
}
...
void free_mode_pred_info(mode_pred_info *mpi) {
  ...
  if(mpi->MVx)              free(mpi->Intra4x4PredMode);
  if(mpi->MVy)              free(mpi->Intra4x4PredMode);
  free(mpi);
}
...
static inline mv get_MV(mode_pred_info *mpi, int x, int y) {
 ...
 // printf ("111: res (%d,%d)\n",res.x,res.y);
  if(res.x==MV_NA) {	
    res.x=0;
    res.y=0;
	printf ("222 IsIntra(ModePredInfo_MbMode = %d\n",IsIntra(ModePredInfo_MbMode(mpi,x>>2,y>>2)));
 ...
 // printf ("333 res.available = %d      res.valid = %d\n",res.available,res.valid);
  return res;
}
