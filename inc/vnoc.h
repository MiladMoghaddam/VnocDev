...
class VNOC {
    private:
		...
		//milad add
		long _NOC_total_packet_injected;

		long _GET_NALU_PE_total_packet_injected;
		long _GET_NALU_PE_packet_injected_num;
		
		long _DECODE_HDR_PE_total_packet_injected;
		long _DECODE_HDR_PE_packet_injected_num;

		long _DECODE_MB_PE_total_packet_injected;
		long _DECODE_MB_PE_packet_injected_num;

		long _INTRA_PRED_PE_total_packet_injected;
		long _INTRA_PRED_PE_packet_injected_num;
		//long _INTRA_PRED_PE_secondary_packet_injected_num;

		long _INTER_PRED_PE_total_packet_injected;
		long _INTER_PRED_PE_packet_injected_num;		

		long _FRAME_BUFFER_PE_total_packet_injected;
		long _FRAME_BUFFER_PE_packet_injected_num;

		long _DISPLAY_PE_total_packet_injected;
		long _DISPLAY_PE_packet_injected_num;
    ...
    public:
		...
		//milad add        
		vector<NOC_interface_send> _noc_ifc_send;
		//vector<NOC_interface_send> _noc_ifc_send_secondary;
        vector<NOC_interface_receive> _noc_ifc_receive;

        obj_get_nalu_PE _nalu_PE;
		obj_decode_hdr_PE _decode_hdr_PE;
		obj_decode_MB_PE _decode_MB_PE;
		obj_intra_pred_PE _intra_pred_PE;
		obj_inter_pred_PE _inter_pred_PE;
		obj_frame_buffer_PE _frame_buffer_PE;
		obj_display_PE _display_PE;
    ...
    public:
		...
		//milad add
		bool receive_EVENT_TO_GET_NALU_FIRST_RUN(EVENT this_event);
		bool receive_EVENT_TO_GET_NALU(EVENT this_event);
		bool receive_EVENT_GET_NALU_TO_DECODE_HDR_FIRST_RUN(EVENT this_event);
		bool receive_EVENT_GET_NALU_TO_DECODE_HDR(EVENT this_event);
		bool receive_EVENT_DECODE_HDR_TO_DECODE_MB_FIRST_RUN(EVENT this_event);
		bool receive_EVENT_DECODE_HDR_TO_DECODE_MB(EVENT this_event);
		bool receive_EVENT_DECODE_MB_TO_INTRA_PRED(EVENT this_event);
		bool receive_EVENT_DECODE_MB_TO_INTER_PRED_NORMAL(EVENT this_event);
		bool receive_EVENT_DECODE_MB_TO_INTER_PRED_PSKIP(EVENT this_event);
		bool receive_EVENT_INTRA_PRED_MB_FRAME_TO_INTER_PRED(EVENT this_event);
		bool receive_EVENT_INTER_PRED_MB_FRAME_TO_INTRA_PRED(EVENT this_event);
		bool receive_EVENT_INTRA_PRED_MB_FRAME_TO_FRAME_BUFFER(EVENT this_event);
		bool receive_EVENT_INTER_PRED_MB_FRAME_TO_FRAME_BUFFER(EVENT this_event);
		bool receive_EVENT_FRAME_BUFFER_MB_FRAME_TO_DISPLAY(EVENT this_event);
		bool receive_EVENT_TO_CONTINUE_DECODE_MB(EVENT this_event);
		bool receive_EVENT_UPDATE_REF_FRAME_IN_INTER_PRED(EVENT this_event);		
		bool receive_EVENT_UPDATE_FRAME_IN_FRAME_BUFFER(EVENT this_event);
        ...
        void frame_number_plus_plus(){_frame_number++;}
		...
		//miald add
		bool is_it_last_packet(DATA packet,int packet_size);
		long get_packet_injected_num(string module_name);
		long get_total_packet_injected_num(string module_name);
};
