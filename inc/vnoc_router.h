...
class FLIT {
    ...
    public:
        ...
        void Display()
        {
          cout <<_id <<"-"<<_type <<"-("<<_src_addr[0]<<","<<_src_addr[1]<<")-("<<_des_addr[0]<<","<<_des_addr[1]<<")-"<<_start_time<<"-"<<_data[0]<<endl;   
...
class ROUTER {
    ...
    public:
        ...
        //milad add
        //void set_data_flit(DATA data_in) 
        //{
        //    _init_data = data_in;
        //}
        //milad add
        long receive_packet_from_local_IP_CORE(DATA packet_data);
        void inject_packet_by_IP_CORE( long flit_id, ADDRESS &sor_addr, ADDRESS &des_addr,
                                        double time, long packet_size , DATA packet_data);


        long receive_packet_from_local_PE(DATA packet_data, ADDRESS &des_addr);
        void inject_packet_by_PE( long flit_id, ADDRESS &sor_addr, ADDRESS &des_addr,
                                        double time, long packet_size , DATA packet_data);
